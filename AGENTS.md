...
- Superseded: the tap cask's `postflight` quarantine strip (added 2026-07-07 as the free workaround for the "damaged" error) is now redundant — a notarized app passes Gatekeeper with the quarantine flag intact. Remove it from `Casks/dblitz.rb` once a notarized release is verified in the wild; it is harmless until then.
